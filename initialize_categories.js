...
// This should work when Firebase CLI is authenticated
admin.initializeApp({
...
async function initializeCategories() {
  console.log('🚀 Starting to initialize 26 comprehensive categories...');
  ...
  try {
    ...
    if (!existingCategories.empty) {
      console.log(`⚠️  Found ${existingCategories.size} existing categories. Clearing them first...`);
    ...
    // Add new categories
    console.log('📝 Adding 26 new categories...');
    const batch = db.batch();
    ...
    console.log(`✅ Successfully initialized ${newCategories.size} categories!`);
    
    // List all categories for verification
    console.log('\n📋 Categories created:');
    newCategories.docs.forEach((doc, index) => {
      const data = doc.data();
      console.log(`${index + 1}. ${data.name} (${data.slug})`);
    });
