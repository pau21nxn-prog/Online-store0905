...
class CategoryService {
  ...
  static Future<List<Category>> getCategoriesByParent(String? parentId) async {
  ...
  static Future<String> createCategory({
    ...
    // Determine level and sort order
    int level = 0;
    int sortOrder = 0;
    ...
    if (parentId != null) {
      ...
      if (parent != null) {
        ...
        final updatedChildIds = List<String>.from(parent.childIds);
        final newDocRef = _firestore.collection(_collection).doc();
        updatedChildIds.add(newDocRef.id);
        
        await updateCategory(parent.copyWith(childIds: updatedChildIds));
      }
      
      // Get sort order as last child
      final siblings = await getCategoriesByParent(parentId);
      sortOrder = siblings.length;
    } else {
      // Get sort order as last top-level category
      final topLevel = await getTopLevelCategories();
  ...
  static Future<void> deleteCategory(String id) async {
    ...
    await _deleteSubcategoriesRecursively(id, batch);

    // Remove from parent's childIds if it has a parent
    if (category.parentId != null) {
      final parent = await getCategory(category.parentId!);
      if (parent != null) {
        final updatedChildIds = List<String>.from(parent.childIds);
        updatedChildIds.remove(id);
        batch.update(
          _firestore.collection(_collection).doc(category.parentId!),
          {'childIds': updatedChildIds, 'updatedAt': FieldValue.serverTimestamp()},
        );
      }
    }
  ...
  static Future<void> _deleteSubcategoriesRecursively(String parentId, WriteBatch batch) async {
    final subcategories = await getCategoriesByParent(parentId);
    
    for (final subcategory in subcategories) {
      // Recursively delete children
      await _deleteSubcategoriesRecursively(subcategory.id, batch);
      
      // Delete the subcategory
      batch.delete(_firestore.collection(_collection).doc(subcategory.id));
    }
  }

  // Initialize categories from CategoryList.md structure
  static Future<void> initializeCategoriesFromList() async {
    final categories = _getCategoryListStructure();
  ...
  // Get the comprehensive category structure from CategoryList.md
  static List<Map<String, dynamic>> _getCategoryListStructure() {
  ...
  static Future<List<Category>> getCategoryHierarchy() async {
